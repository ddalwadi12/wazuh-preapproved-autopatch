...
└── repository/
    ├── README.md
    └── example/
        ├── ubuntu/example-package/
        │   ├── metadata.json
        │   └── playbook.yml
        └── windows/example-package/
            ├── metadata.json
            └── playbook.yml
```
...
- Python 3.10+
- Wazuh Indexer / OpenSearch access
- OpenSSH client for `scp`
- Ansible
- SSH access for Linux targets
- WinRM access for Windows targets

For Windows Ansible support:
...
## Approved patch metadata

Each package requires a `metadata.json` similar to:
...
```json
{
  ...
  "cves": ["CVE-2099-0001"],
  "patch_file": "example-installer.msi",
  "patch_sha256": "<sha256>",
  "fixes_version": "2.0.0",
...
```bash
sha256sum example-installer.msi
```

## Important production considerations

This repository is suitable as a lab/portfolio implementation and a foundation
for a more mature patching platform. Before using the design in production,
consider adding:

- a proper secrets manager such as HashiCorp Vault, Azure Key Vault or Ansible
  Vault;
- TLS certificate verification for Wazuh;
- HTTPS/authenticated artifact hosting instead of plain HTTP;
- maintenance windows and reboot handling;
- rollback logic;
- deployment rings/canary groups;
- structured audit logging;
- ticket/change-management integration;
- post-patch vulnerability rescanning and compliance verification;
